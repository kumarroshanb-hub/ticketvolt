...
class TicketGenerator:
    ...
    def generate_ticket_image(self, ticket, booking, event, custom_config=None):
        ...
        try:
            ...
            if not os.path.exists(img_path):
                logger.error(f"❌ Template image not found: {img_path}")
                return self._generate_fallback_ticket(ticket, booking, event)
            ...
            # 1. Event Title - Position based on template size
            title_y = int(height * 0.28) if height > 500 else 130
            self._draw_text_px(draw, img, event.title.upper(), width//2, title_y, 32, '#D69E2E', 'center')
            ...
            self._draw_text_px(draw, img, date_display, width//2, date_y, 34, '#1A202C', 'center')
            ...
            if event and event.end_date and event.start_date:
                ...
                if start_date_only != end_date_only:
                    date_secondary = timezone.localtime(event.start_date).strftime('%B %Y').upper()
            sec_y = int(height * 0.45) if height > 500 else 235
            self._draw_text_px(draw, img, date_secondary, width//2, sec_y, 22, '#4A5568', 'center')
            
            # ============================================
            # TICKET DETAILS - Position based on template size
            # ============================================
            ...
            for i, line in enumerate(venue_lines):
                self._draw_text_px(draw, img, line, label_x, venue_y + 25 + (i * 22), 14, '#2D3748', 'left')
    ...
    def _generate_fallback_ticket(self, ticket, booking, event):
        ...
        try:
            ...
            # Draw simple ticket with border
